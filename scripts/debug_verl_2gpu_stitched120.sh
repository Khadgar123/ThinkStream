#!/bin/bash
# 2-GPU verl GRPO debug — stitched MAX_TURNS=120 baseline.
# Verifies that the streaming agent loop can handle up to 120 turns
# without prompt-budget aborts, and FSDP can process longer stitched
# sequences (~8-16K) on 2× H20 with offload.
set -euo pipefail
...
# v12.14: stitched mode (default)
export THINKSTREAM_RECURRENT_MODE="stitched"
...
# ── Capacity config for MAX_TURNS=120 stitched baseline ──
# Synthetic data has ~30 chunks; real batch1 has 60-95.
# 16384 covers ~60 chunks per run_thinkstream_grpo.sh comments.
MAX_MODEL_LEN=16384
RESP_LEN=8192
PROMPT_LEN=8192

# 2-GPU FSDP needs ppo_max_token_len_per_gpu >= MAX_MODEL_LEN
