...
Usage:
    ...
    python -m scripts.agent_data.pipeline stress_test \
        --api_base http://AMD_IP:8000/v1
"""
...
def _write_batch_manifest(videos: List[Dict], *, source: str, seed: int) -> None:
    ...
    manifest = {
        ...
        "final_files": {
            "sft_trajectories": str(FINAL_DIR / "train_sft_trajectories.jsonl"),
        ...
        "rendered_dirs": {
            "trajectory": str(DATA_ROOT / "rendered" / "trajectory"),
            CANONICAL_RENDER_DIRNAME: str(DATA_ROOT / "rendered" / CANONICAL_RENDER_DIRNAME),
        },
        "derived_files": {
            "sft_trajectory": str(
                DATA_ROOT / "rendered" / "trajectory" / "train_sft_trajectory.jsonl"
            ),
            "val_trajectory": str(
                DATA_ROOT / "rendered" / "trajectory" / "val_trajectory.jsonl"
            ),
            "test_trajectory": str(
                DATA_ROOT / "rendered" / "trajectory" / "test_trajectory.jsonl"
            ),
            "train_parquet": str(
...
async def run_pipeline(
    ...
    from scripts.agent_data.config import FPS, FRAMES_PER_CHUNK
    ...
    # Chain pass4 (trajectory grouping) + pass5 (multi-turn trajectory
    # rendering) so a single pipeline invocation produces every file the
    # downstream trainers actually default to:
    #   - SFT default reads rendered/trajectory/train_sft_trajectory.jsonl
    #   - RL  default reads final/train_rl_trajectories.jsonl / multi-Q parquet
    ...
    if not os.environ.get("SKIP_PASS45"):
        ...
        pass4_ok = False
        try:
            from scripts.agent_data import pass4 as _pass4_mod
        ...
        # rendering pass5 variants so split files and trajectory rows stay in
        # sync.
        try:
            from scripts.agent_data import rebalance_mc_options as _mc_mod
        ...
        # ── pass5 trajectory rendering ──
        # Default-on for the compact-memory pipeline. It reads the same FINAL_DIR
        # trajectory JSONL files and emits Qwen-native messages+tools format to
        # rendered/trajectory/. Set THINKSTREAM_RUN_PASS5_TRAJECTORY=0 to skip.
        pass5_ok = False
        _run_trajectory = (
            os.environ.get(
                "THINKSTREAM_RUN_PASS5_TRAJECTORY",
                os.environ.get("THINKSTREAM_RUN_PASS5_V2", "1"),
            ) != "0"
        )
        if pass4_ok and _run_trajectory:
            logger.info("=" * 60)
            logger.info("PASS 5: multi-turn + multi-trajectory rendering")
            logger.info("=" * 60)
            try:
                from scripts.agent_data import pass5 as _pass5_mod
                trajectory_output_dir = DATA_ROOT / "rendered" / "trajectory"
                trajectory_output_dir.mkdir(parents=True, exist_ok=True)
                manifest = _pass5_mod.convert_dir(
                    input_dir=FINAL_DIR,
                    output_dir=trajectory_output_dir,
                    frames_root=None,  # frame paths come from pass3 samples;
                                       # SFT loader resolves them at load time.
                )
                logger.info(
                    "PASS 5 trajectory manifest: %d splits, %d rows total",
                    len(manifest),
                    sum(m.get("rows_out", 0) for m in manifest.values()),
                )
                ...
                logger.error(
                    "pass5 trajectory failed: %s; SFT/eval trajectory datasets may be missing",
                    e,
                )
        ...
        logger.info("=" * 60)
        try:
            from scripts.agent_data import build_verl_parquet as _parquet_mod
        ...
        if pass5_ok:
            logger.info(
                "Rendered SFT/eval trajectories: %s",
                DATA_ROOT / "rendered" / "trajectory",
            )
    ...
    logger.info("Next:")
    logger.info("  SFT: bash scripts/sft_trajectory.sh")
