"""vLLM smoke test on any checkpoint with batch1 samples."""
import argparse
import json
import sys
...
def main():
    ...
    parser.add_argument("--data", default=str(ROOT / "data/batch1_backup/final_batch1_backup/val.jsonl"))
    ...
    samples = load_samples(args.data, n=args.samples)
    ...
    for i, sample in enumerate(samples):
        ...
        print(f"Gold:      {sample['output'][:500]}")
        ...
        if parsed:
            ...
            if parsed.get('summary'):
                print(f"Parsed summary: {parsed['summary'][:200]}")
            if parsed.get('response'):
                print(f"Parsed response: {parsed['response'][:200]}")
        print()
