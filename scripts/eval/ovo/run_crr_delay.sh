#!/bin/bash
#
# OVO CRR delay-aware eval — uses ORIGINAL ovo_bench_new.json so
# ask_time / clue_time / test_info structure is preserved (the formatted
# version collapses these into single-shot QA, which destroys what CRR
...
#   fp_rate     - type=0 probes where agent said "Yes" (the real
#                 failure mode that strict eval cares about)
#
# Retriever modes:
...
#
# Usage:
#   bash scripts/eval/ovo/run_crr_delay.sh \
...
mkdir -p "${OUT_DIR}"
OUT_JSON="${OUT_DIR}/${TASK}_${RETRIEVER}.json"

echo "=========================================="
echo "OVO ${TASK} delay-aware eval"
echo "  ckpt:       ${CKPT}"
