...
SPLIT_POLICY=${SPLIT_POLICY:-continuous_prefix}
...
if [[ "${SPLIT_POLICY}" == "continuous_prefix" ]]; then
    echo "  pack tasks: true (forced by continuous_prefix)"
else
    echo "  pack tasks: ${PACK_ACROSS_TASKS}"
fi
echo "  split:      ${SPLIT_POLICY}"
echo "  short span: ${SHORT_MIN_SPAN_CHUNKS}-${SHORT_MAX_SPAN_CHUNKS}"
echo "  compress:   ${THINKSTREAM_RL_COMPRESS_TRIGGER_SOURCE:-offline_pass2_boundaries} planned-range=${THINKSTREAM_COMPRESS_RANGE_MIN:-25}-${THINKSTREAM_COMPRESS_RANGE_MAX:-45}"
...
export THINKSTREAM_COMPRESS_THRESHOLD="${THINKSTREAM_COMPRESS_THRESHOLD:-3200}"
export THINKSTREAM_COMPRESS_RANGE_MIN="${THINKSTREAM_COMPRESS_RANGE_MIN:-25}"
export THINKSTREAM_COMPRESS_RANGE_MAX="${THINKSTREAM_COMPRESS_RANGE_MAX:-45}"
