...
if [[ "${FRAME_PROTOCOL}" != "video_meta" ]]; then
    echo "ERROR: canonical OVO RL eval uses FRAME_PROTOCOL=video_meta" >&2
...
echo "  diff <(jq .summary.category <SFT>/eval/ovo_full/sft_${RETRIEVER}_compress-system_${FRAME_PROTOCOL}.json) \\"
