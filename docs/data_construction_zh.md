...
FAMILY_TARGETS = {
    ...
    "E2": 2,   # State change — event_watch 的唯一来源
    "P1": 2,   # Procedure — 优先 multiple_choice/number
    "C1": 2,   # Comparison — 优先 binary
    "R1": 1,   # Re-identification — 天然 binary
    "S1": 2,   # Summary — 唯一允许 descriptive 的 family
}
# 每视频目标 ~23 个 task cards
```
...
| **M1** | 连续解说 | 实时解说模式 | — | — | **单独 commentary mode** |

> D1/M1 不进一版：连续解说和多轮描述会拉高 response 先验，破坏 recall/silent 的判断边界。
...
**Pending → 只保留 E2 event_watch**：
- 只有 E2 family + in_future availability 触发
- 只产生 2 个样本（silent at ask + response at trigger），不再有中间 pending_silent
- 不设 pending 队列，不设 query-response 存储
...
### 9.1 Pending Question 状态表示（v8.0 简化）

> v8.0: Pending 只保留 E2 event_watch 一种，简化为 2 样本（不再有中间 pending_silent）。
> 不设 pending 队列，不设 query-response 存储。think 本身就是状态记录。

**Event watch 在 memory block 中的表示**：

```
[Memory block:]
  <compressed>{"time_range":[0,20],"text":"..."}</compressed>
  [20-22] think...
  <pending since="24">Tell me when the chef adds the basil.</pending>
```

**只产生 2 个样本**：

1. **silent at ask_time**：用户提出 event-watch 请求，事件还没发生 → silent
   ```
   <think>Sauce still simmering, no basil yet.</think><action>silent</action>
   ```

2. **response at trigger_time**：事件发生 → response
   ```
   <think>Chef tearing basil leaves over pot.</think><action>response</action><response>The chef is now adding basil.</response>
   ```

不再有 pending_silent 中间样本。模型只需要学会"等"和"触发"两个边界。
...
### 9.6 Pending-question-aware vs Question-blind Compression（v8.0 简化）

> v8.0: Pending 简化为仅 event_watch，因此 pending-aware 压缩场景大幅减少。

**场景 A: Question-blind compression**（绝大多数情况）
```
[thinks 0-20]    → compress → summary 按通用重要性保留
不知道未来问什么，所有类似动作用相同粒度压缩
```

**场景 B: Event-watch-aware compression**（仅当有 E2 pending 时）
```
<pending>"Tell me when basil is added"</pending>
[thinks 20-40]   → compress → summary 保留 basil 相关信息
```

**训练数据中两种比例**：
- Question-blind: 90%
- Event-watch-aware: 10%（仅当视频有 E2 event_watch 任务时）
...
| 8 | action 优先���：用户交互 > pending > compress > silent | §2.1 | pipeline Pass 3-C `interaction_chunks` 优先级过滤 |
