...
> 目标: 312 个视频实测落盘（v9.0 规划 319，8 条 num_chunks=0 自动 drop），
> SFT + GDPO RL 两阶段，覆盖所有 task type
...
> ⚠️ **§1-§7 是 v9.0 规划估算，绝对值已被 v11.1 actuals（本文档顶部）取代**。
> 这些章节保留是因为 per-video 配比、时长分桶、问题家族策略仍然有效——
> 看比例、不看绝对值。

| 指标 | 数值 |
|------|------|
| 视频总数 | 319（原 320，移除 1 条坏记录）→ **v11.1 实测 312**（多 8 条 num_chunks=0 自动 drop） |
| 总视频时长 | ~14.1 小时 |
...
> ⚠️ v11.1: **C2 已删除**（`pipeline.py:assign_phase` 不再产出 "C2" 标签，
> `c2_train.jsonl` 是空文件已删除）。Phase 数据文件**只用于诊断/分桶 eval**，
> 不构成训练 curriculum。生产训练单 SFT (`train_sft.jsonl`) + 单 GDPO RL
> (`train_rl.jsonl`)。下表是 v9.0 估算，实测见顶部 v11.1 actuals。
...
| C1 (指定范围压缩) | ~964 | 14% |
| ~~C2 (自选压缩)~~ | ~~~757~~ | **删除**（移到 RL 的 overflow_pen reward） |
