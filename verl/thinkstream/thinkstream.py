...
def compute_score(
    ...
    # verl's reward_manager passes num_turns through extra_info — that's
    # the most reliable answer-chunk index. Falls back to "the last chunk
    # that contains <answer>" if the rollout adapter didn't fill it in.
    answer_chunk: Optional[int] = None
    n_turns = extra.get("num_turns")
    if final_answer is not None:
        if isinstance(n_turns, int) and n_turns > 0:
            answer_chunk = n_turns - 1
        else:
