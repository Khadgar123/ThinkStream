...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        # Full params
        if rollout_name == "streaming" and fsdp_version(self.actor_module_fsdp) == 1:
            # The ThinkStream true-KV HF backend runs one full local rollout
            # model per GPU. Each rank therefore needs a full state dict for
            # its colocated rollout server instead of the sharded state dict
            # used by tensor-parallel inference engines.
            FSDP.set_state_dict_type(
                self.actor_module_fsdp,
                state_dict_type=StateDictType.FULL_STATE_DICT,
                state_dict_config=FullStateDictConfig(rank0_only=False),
            )
        elif torch.distributed.get_world_size() == 1 and fsdp_version(self.actor_module_fsdp) == 1:
            FSDP.set_state_dict_type(
                self.actor_module_fsdp,
                state_dict_type=StateDictType.FULL_STATE_DICT,
