...
def compute_format_reward(assistant_outputs: List[str]) -> float:
    ...
    terminal (</Silence>, </Response> text, <tool_call>, or compact <m> lines), no extra
    text outside those tags, and strict recall/compress tool JSON schema.
    """
...
def compute_trajectory_outcome(
    ...
    *,
    ...
    Args:
      ...
      answer_form_judge: optional callable(model_answer, gold_answer,
        answer_form) → float ∈ [0,1]. If None, uses compute_outcome_reward
    ...
    for q in trajectory_questions:
        ...
        for i, gold_event in enumerate(gold_events):
            ...
            if answer_form_judge is not None:
                ...
                ask_score = compute_outcome_reward(
...
def compute_per_chunk_silent_quality(
