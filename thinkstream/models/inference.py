...
class StreamingInferenceEngine:
    ...
    def reset_to_prefix(
        ...
        """Partial reset: keep the first ``keep_lengths[i]`` cache tokens for
        batch item ``i`` and drop the rest. Used at trajectory boundaries
        (compress events) to retain the system prompt + carried-over memory
        summary while discarding the previous trajectory's chunk content.

        Args:
            keep_lengths: [batch_size] long tensor; for each item, the number
                of KV cache positions to keep from the start.
            next_start_pos: replacement value for ``self.next_start_pos``,
                which anchors position-id shifting on the next ``generate``.
                Shape must match what the existing ``next_start_pos`` would
                hold ([B, 1] for standard, [3, B, 1] for MROPE).

        Side effects:
            - Truncates ``cache.cache_seqlens`` to ``keep_lengths`` across all
              layers (the underlying K/V tensors are not zeroed; subsequent
              writes overwrite stale entries by index).
            - Replaces ``self.next_start_pos`` for downstream position-shift.
        """
        assert keep_lengths.shape[0] == self.batch_size, (
            f"keep_lengths shape {keep_lengths.shape} mismatch with batch_size {self.batch_size}"
        )
        cur = self.decoder.cache.cache_seqlens  # [num_layers, batch_size]
        target = keep_lengths.to(cur.dtype).to(cur.device)
        delta = target.unsqueeze(0) - cur  # broadcast across layers
        # cache.adjust_seqlens expects per-batch delta and broadcasts across
        # layers when layer_idx=None. Apply layer-by-layer to honor the
        # per-layer cache_seqlens invariant in case a layer is desynced.
    ...
    def sample_one_step(
        ...
        *,
        ...
        log_prob_out: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        # Wave 3.1: record per-token log-prob of the FINAL chosen token under
        # the (post temperature / repetition-penalty) policy distribution. RL
        # actor needs this for importance sampling / PPO ratio. Forced tokens
        # (e.g. think-budget overrides) get the policy's log P of the forced
        # token, which is the correct quantity for PPO.
        if log_prob_out is not None and step is not None:
            log_probs_full = torch.log_softmax(logits, dim=-1)
            chosen_log_prob = log_probs_full.gather(1, next_token).squeeze(-1)
            log_prob_out[:, step] = chosen_log_prob
        return next_token
    ...
    def generate(
        ...
        *,
        ...
        Multi-turn: a second call to ``generate`` on the same engine instance
        will append to the existing KV cache (anchored at ``self.next_start_pos``).
        Pass the new user turn's tokens + position_ids (starting at 0; will be
        shifted by ``next_start_pos`` automatically) to continue the dialogue.
        ...
        Args:
            ...
            return_log_probs: When True, also return per-token log probabilities
                under the (post temperature / repetition-penalty) policy
                distribution. RL actor needs these for PPO/GRPO importance
                sampling. Returns ``(tokens, log_probs)`` tuple with each
                element a list of [seq_len] tensors aligned with ``tokens``.
        """
        ...
        # Wave 3.1: pre-allocate per-step log-prob buffer when caller asks for
        # log_probs (RL rollout). 0.0 padding for unused steps; mask via
        # valid_token_lens when slicing at return time.
        log_prob_buffer: Optional[torch.Tensor] = None
        if return_log_probs:
            log_prob_buffer = torch.zeros(
                (effective_bsz, max_new_tokens + 1),
                dtype=torch.float32,
                device=self.device,
            )
        ...
        if return_log_probs:
            log_probs_out = [
                log_prob_buffer[i, :length]
                for i, length in enumerate(valid_token_lens.tolist())
            ]
            return tokens_out, log_probs_out
        return tokens_out
...
class StreamingWindowInferenceEngine(StreamingInferenceEngine):
    ...
    def reset_to_prefix(
        ...
        """Partial reset preserving an arbitrary prefix.

        Video-window bookkeeping is rewound to retain the first
        ``keep_window_count[i]`` entries per batch item. Entries past that
        count are zeroed (out of band; never read when ``_window_count`` is
        below them). If ``keep_window_count`` is None, defaults to all-zero
        (assume the kept prefix has no archived video chunks, e.g. system +
        ...
        assert keep_window_count.shape[0] == self.batch_size, (
            f"keep_window_count shape {keep_window_count.shape} mismatch with batch_size {self.batch_size}"
        )
        # Mask out entries at indices >= keep_window_count per row.
        idx_grid = torch.arange(self.video_flex_window_size, device=self.device)
        keep_mask = idx_grid.unsqueeze(0) < keep_window_count.unsqueeze(1)  # [B, W]
        self._window_starts = torch.where(
            keep_mask, self._window_starts, torch.zeros_like(self._window_starts)
        )
...
def tool_call_stop_sample(
    ...
    **kwargs,
    ...
    """Sample wrapper that forces EOS the step after ``</tool_call>`` is emitted.

    Used during RL rollout to end the assistant turn cleanly at the close of
    a tool call, without burning further tokens. The Qwen chat template would
    normally have the model emit ``<|im_end|>`` after the tool_call block, but
    a noisy actor may continue generating. Forcing EOS here keeps the
    trajectory tight.

    Composable with another sample callback via ``inner`` / ``inner_kwargs``:
    the inner callback runs first (e.g. think_budget_sample to enforce a
    thinking budget), then this wrapper inspects the result and forces EOS if
    the previous step produced the tool_call close token.

    Args:
        tool_call_close_token_id: token id for the ``</tool_call>`` close
            marker. Resolve via
            ``tokenizer.convert_tokens_to_ids("</tool_call>")`` or
            equivalent depending on tokenizer special-token registration.
        eos_token_id: primary EOS token id (typically ``<|im_end|>``).
        inner: optional inner sample callback applied first.
        inner_kwargs: kwargs forwarded to the inner callback.
    """
    ...
    prev = generated_tokens[:, step - 1]  # [B]
    force_eos = prev == tool_call_close_token_id  # [B]
    if force_eos.any():
        eos = torch.full_like(next_token, eos_token_id)
        next_token = torch.where(force_eos.unsqueeze(1), eos, next_token)
    return next_token
